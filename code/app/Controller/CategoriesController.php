...
class CategoriesController extends AppController {
    ...
    public function admin_index() {
        ...
        if (!empty($this->data)) {
            ...
            }if (isset($this->data['Category']['type']) && $this->data['Category']['type'] != '') {
                $type = trim($this->data['Category']['type']);
            }
            ...
            if (isset($this->params['named']['type']) && $this->params['named']['type'] != '') {
                $type = urldecode(trim($this->params['named']['type']));
            }
        }
        ...
        if (isset($type) && $type != '') {
            $separator[] = 'type:' . urlencode($type);
            $condition[] = " (Category.type like '" . addslashes($type) . "')  ";
            $this->set('type', $type);
        }
    ...
    public function admin_add() {
        ...
        if ($this->data) {
            ...
            }elseif ($this->Category->isRecordUniqueCategory($this->data["Category"]["name"],0,$this->data["Category"]["type"]) == false) {
                $msgString .="- Category Name already exists.<br>";
            }
            
            if ($this->data["Category"]["type"] == "") {
                $msgString .="- Category Type is required field.<br>";
            }
    ...
    public function admin_edit($slug = null) {
        ...
        $msgString = '';
        if(empty($this->Category->findBySlug($slug))){
        ...
        if ($this->data) {
            ...
            if (empty($this->data["Category"]["name"])) {
                ...
                if ($this->Category->isRecordUniqueCategory($this->data["Category"]["name"],0,$this->data["Category"]["type"]) == false) {
                    $msgString .="- Category Name already exists.<br>";
                }
                $this->request->data['Category']['slug'] = $this->stringToSlugUnique($this->data["Category"]["name"],'Category');
            }
   ...
   public function admin_subcategory($cslug=null) {
    ...
    public function getSubCategoryList($model = 'User', $class = null){
        $this->layout = '';
        $catId = $this->data[$model]['category_id'];
        $subCategoryList = $this->Category->getSubCategoryList($catId);
