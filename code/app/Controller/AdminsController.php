...
class AdminsController extends AppController {
    ...
    var $uses = array('Admin', 'User', 'Emailtemplate', 'Setting', 'Industry', 'Category', 'Product', 'Service', 'State');
    ...
    public function admin_dashboard() {
        ...
        $countCategory = $this->Category->find('count', array('conditions' => array('Category.parent_id' => '0')));
        $this->set('countCategory', $countCategory);
        
        $countProduct = $this->Product->find('count', array('conditions' => array()));
        $this->set('countProduct', $countProduct);
        $countService = $this->Service->find('count', array('conditions' => array()));
        $this->set('countService', $countService);
        $countState = $this->State->find('count', array('conditions' => array()));
        $this->set('countState', $countState);
    ...
    public function admin_companyChart($dayCount=2){ 
        ...
        if($dayCount == 365){
            ...
            if(date('d') == 1){
                $countUserArray1 = $this->User->find('all', array('conditions' => array('User.created <='=>$today,'User.created >='=>date('Y-m-d 00:00:00') ), 'fields' => array('User.created', 'COUNT(*) as count') , 'order' => array('User.created ASC'), 'group'=>array('DAY(User.created)')));
                if($countUserArray1){
                   $CTempArray[ date("Y-m", strtotime($countUserArray1[0]['User']['created']))] = $countUserArray1[0]['0']['count']; 
                }
            }
