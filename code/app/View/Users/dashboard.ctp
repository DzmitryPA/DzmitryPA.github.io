<style>
    .progress-bar-cusomes .rotate{clip: rect(0 60px 119px 0) !important;}
</style>
...
<div class="right_part">
    <div class="aside_contaner_full_ne aside_contaner_full_ne_dks">
        <div class="pad_of_titles pad_of_titlesdashboard">
            ...
            $aboutUs = ($userInfo['User']['about_us'] != "") ? 10 : 0;
            $invitedSupplier = 0;
            $invitedCustomer = 0;
                                ...
                                <div class="bose_dash_full_content bose_dash_full_content_msyskf">
                                    <div class="bose_dash_full_contentsmale_fill" id="content-1">
                                        <div class="bose_dash_full_contentsmale_fill_tab">
                                            ...
                                            <?php
                                            if ($countUserArray) {
                                                foreach ($countUserArray as $followers) {
                                                    ?>
                                                    <div class="bose_dash_full_contentsmale_fill_row">
                                                        <div class="bose_dash_full_contentsmale_fill_cell">
                                                            <div class="bose_dash_full_contentsmale_fill_cellsed">
                                                                <?php
                                                                $filePath = UPLOAD_LOGO_PATH . $followers['Follower']['company_logo'];
                                                                if (file_exists($filePath) && $followers['Follower']['company_logo']) {
                                                                    echo $this->Html->link($this->Html->image(DISPLAY_LOGO_PATH . $followers['Follower']['company_logo'], array('alt' => 'Img')), array('controller' => 'users', 'action' => 'profile', $followers['Follower']['slug']), array('escape' => false, 'class' => ''));
                                                                } else {
                                                                    echo $this->Html->link($this->Html->image('no_image.gif'), array('controller' => 'users', 'action' => 'profile', $followers['Follower']['slug']), array('escape' => false, 'class' => ''));
                                                                }
                                                                ?>
                                                            </div>
                                                        </div>
                                                        <div class="bose_dash_full_contentsmale_fill_cell"><span class="title_of_pahs"><?php echo $followers['Follower']['company_name'] ?></span></div>
                                                        <div class="bose_dash_full_contentsmale_fill_cell"><span class="title_of_uid">@<?php echo $followers['Follower']['unique_id'] ?></span></div>
                                                    </div>
                                                    <?php
                                                }
                                            } else {
                                                echo "<span class='dashboard_no_rec_txt'>No Follower in Past 30 Days</span>";
                                            }
                                            ?>
                                ...
                                <div class="bose_dash_full_content bose_dash_full_content_msyskf">
                                    <div class="bose_dash_full_contentsmale_fill" id="content-2">
                                        <div class="bose_dash_full_contentsmale_fill_tab">
                                            ...
                                            <?php
                                            if ($watchCompanyList) {
                                                foreach ($watchCompanyList as $watch) {
                                                    ?>
                                                    <div class="bose_dash_full_contentsmale_fill_row">
                                                        <div class="bose_dash_full_contentsmale_fill_cell">
                                                            <div class="bose_dash_full_contentsmale_fill_cellsed">
                                                                <?php
                                                                $filePath = UPLOAD_LOGO_PATH . $watch['FromWatch']['company_logo'];
                                                                if (file_exists($filePath) && $watch['FromWatch']['company_logo']) {
                                                                    echo $this->Html->image(DISPLAY_LOGO_PATH . $watch['FromWatch']['company_logo'], array('alt' => 'Img'));
                                                                } else {
                                                                    echo $this->Html->image('no_image.gif');
                                                                }
                                                                ?>
                                                        ...
                                                        <div class="bose_dash_full_contentsmale_fill_cell"><span class="title_of_pahs"><?php echo $watch['FromWatch']['company_name'] ?></span></div>
                                                        <div class="bose_dash_full_contentsmale_fill_cell"><span class="title_of_uid"><?php echo date('jS M, Y', strtotime($watch['Watch']['modified'])); ?></span></div>
                                                        <div class="bose_dash_full_contentsmale_fill_cell"><span class="title_of_uid"><?php echo $watch['Industry']['name'] ?></span></div>
                                                    </div>
                                                <?php
                                                }
                                            } else {
                                                echo "<span class='dashboard_no_rec_txt'>No Profile Watch in Past 30 Days</span>";
                                            }
                                            ?>
...
<script>
    ...
    var chartData = [];
    var arrayname = [];
    arrayname = <?php echo $jsonFinalArray ?>;
...
<script>
    $(document).ready(function () {
        var pieData = <?php echo $jsonFinalWatchArray ?>;

        var pie_ctx = document.getElementById("pie-chart-area").getContext("2d");
        window.myPie = new Chart(pie_ctx).Pie(pieData, {
            legendTemplate: "<ul class=\"<%=name.toLowerCase()%>-legend\"><% for (var i=0; i<segments.length; i++){%><li><span style=\"background-color:<%=segments[i].fillColor%>\"></span><em><%if(segments[i].label){%><%=segments[i].label%><%}%></em></li><%}%></ul>"
                    });
...
/*
  ...
  var chartData = <?php echo $jsonFinalWatchArray ?>;
  var convertPerValue = <?php echo $convertPerValue ?>;
  var pieDataColor = ['#90d44c', '#5fb4f1', '#f46554', '#eef455', '#c49ef4'];
  var pieDataHighlight = ['#7fbd41', '#51a3dd', '#dd594a', '#d8de4a', '#ac88db'];
  ...
  ++i;
  {
  value: val * convertPerValue,
  color: pieDataColor[i],
  highlight: pieDataHighlight[i],
  label: key
  },
