<?php if ($timelines) { ?>
    ...
    foreach ($timelines as $timeline) {
            ...
            <div class="timeline_sections_right">
                <div class="timeline_sections_right_top">
                    ...
                    <div class="timeline_sections_right_top_content">
                        ...
                        <?php } ?>
                        <p id="timeline_other_desc_full<?php echo $timeline['Timeline']['id'] ?>" style="display: none"><?php echo $timeline['Timeline']['description'] ?> </p>
                        <p id="timeline_other_desc_less<?php echo $timeline['Timeline']['id'] ?>">
                            <?php if ($timeline['Timeline']['description'] != "") { ?>
                                        <?php
                                    $string = strip_tags($timeline['Timeline']['description']);
                                    
                                    if (strlen($string) > 400) {
                                        $stringCut = substr($string, 0, 400);
                                        $string = substr($stringCut, 0, strrpos($stringCut, ' ')) . '...' . $this->Html->link('Show More', 'javascript:void(0);', array('escape' => false, 'onclick' => 'showMoreOther('.$timeline['Timeline']['id'].')'));
                                    }
                                    echo $string;
                                    ?>
                                    <?php }
                ...
                <div class="timeline_sections_right_footer">
                    <div class="comment_liks"><a onclick="showCommentFormOther(<?php echo $timeline['Timeline']['id'] ?>)" id="comment_shwbutton<?php echo $timeline['Timeline']['id'] ?>"><i class="fa fa-comments" aria-hidden="true"></i> Comment</a></div>
                    <?php if ($timeline['Timeline']['post_type'] == 2) { ?>
                        <?php if ($timeline['Timeline']['user_id'] != $this->Session->read('user_id')) { ?>
                        <div class="make_offer_btn"><a id="makeoffer_popup_other<?php echo $timeline['Timeline']['id'] ?>" onclick="showMakeOfferOther(<?php echo $timeline['Timeline']['id'] ?>)">Make Offer</a></div>
                    <?php } } ?>
                </div>                                
            </div>
            <div class="comment_today_section" id="comment_formdivother<?php echo $timeline['Timeline']['id'] ?>">
                ...
                <div class="btm_loader1 comment_loader_other<?php echo $timeline['Timeline']['id'] ?>"> <?php echo $this->Html->image('loading_bl.svg'); ?></div>
                <script>
                        $(document).ready(function () {
                            $("#postcommentOther<?php echo $timeline['Timeline']['id'] ?>").validate();
                        });
                </script>
                <?php echo $this->Form->create(Null, array('id' => 'postcommentOther' . $timeline['Timeline']['id'])); ?>
                <?php echo $this->Form->textarea('Comment.comment', array('class' => 'com_textareasexcr', 'id' => 'txtara_comm_other'. $timeline['Timeline']['id'], 'placeholder' => 'Write your comment here')); ?>
                <input type="hidden" id="" name="data[Comment][timeline_id]" value="<?php echo $timeline['Timeline']['id'] ?>" >
                <input type="button" class="post_com_btnn" name="comment" id="postcomm_other<?php echo $timeline['Timeline']['id'] ?>" value="Post Comment" onclick="saveDataOther(<?php echo $timeline['Timeline']['id'] ?>);"/>
        ...
        <?php if ($timeline['Timeline']['post_type'] == 2) { ?>
            <div class="popup_full_sectiuons" id="makeoffer_popup_box_other<?php echo $timeline['Timeline']['id'] ?>">
                <div class="popup_insude_box">
                    <div class="popup_close_btn" id="makeoffer_popup_box_close_other<?php echo $timeline['Timeline']['id'] ?>" onclick="hideMakeOfferOther(<?php echo $timeline['Timeline']['id'] ?>)"></div>
                    <h3 class="title_of_popup">Make Offer</h3>
                    <script>
                        $(document).ready(function () {
                            $("#postofferother<?php echo $timeline['Timeline']['id'] ?>").validate({
                                submitHandler: function (form) {
                                    $.ajax({
                                        type: 'POST',
                                        url: '<?php echo HTTP_PATH; ?>/timelines/saveOfferData/',
                                        data: $('#postofferother<?php echo $timeline['Timeline']['id'] ?>').serialize(),
                                        beforeSend: function () {
                                            $("#loaderId").show();
                                        },
                                        success: function (result) {
                                            $("#loaderId").hide();
                                            if (result == 1) {
                                                $('#successmsgother_<?php echo $timeline['Timeline']['id'] ?>').show();
                                                setTimeout(function() { hideMakeOfferOther(<?php echo $timeline['Timeline']['id'] ?>); }, 3000);
                                            } else {
                                                $('#errormsgother_<?php echo $timeline['Timeline']['id'] ?>').show();
                                                setTimeout(function() { hideMakeOfferOther(<?php echo $timeline['Timeline']['id'] ?>); }, 3000);
                                            }
                                        }
                                    });
                    ...
                    <div class="popup_insude_box_ins">
                        <?php echo $this->Form->create(Null, array('id' => 'postofferother' . $timeline['Timeline']['id'])); ?>
                        <div class="ee">
                            <div style="width:100%; display: none; float: left;" id="successmsgother_<?php echo $timeline['Timeline']['id'] ?>" class="alert alert-success successErrorMsg">
                                Your offer has been posted succesfully.
                            </div>
                            <div style="width:100%; display: none; float: left;" id="errormsgother_<?php echo $timeline['Timeline']['id'] ?>" class="alert alert-danger successErrorMsg">
